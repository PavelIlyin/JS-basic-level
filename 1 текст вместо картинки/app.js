...
btnEl.forEach(function(buttons) {
...
function handleClick(clickedButtonEvent) {
    ...
    const textBtnEl = card.button.innerText;

    if (textBtnEl == "Подробнее") { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card);
    } else if (textBtnEl == "Отмена") { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function showMoreText(card) {
    ...
    const textImg = 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Repellendus!';    
    // 6.2 внутри .product есть .productName, после него вставляем div.desc и текстом из переменной из п. 6.1
    card.productName.insertAdjacentHTML('afterend', `<div class="desc">${textImg}</div>`);
